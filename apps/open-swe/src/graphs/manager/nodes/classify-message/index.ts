...
import { FailureHandler } from "../../../../runtime/failure/index.js";
import { getBudgetState } from "../../../../runtime/budget/index.js";
const logger = createLogger(LogLevel.INFO, "ClassifyMessage");

function buildTerminationUpdate(
  state: GraphState,
  ...
  return {
    ...
    terminationMessage: outcome.message,
    qualityFlag: outcome.qualityFlagEmitted ? state.qualityFlag : undefined,
  };
}
...
export async function classifyMessage(
  ...
  const graphState = state as unknown as GraphState;
  const failureHandler = new FailureHandler(getBudgetState(config));

  if ((graphState.reviewerCycleCount ?? 0) >= 2) {
    const loopError = new LoopOverextensionError(
      "manager",
      "manager",
      1,
      graphState.reviewerCycleCount ?? 0,
    );
    const outcome = await failureHandler.dispatch(
      loopError,
      graphState,
      config,
    );
    if (!outcome.resolved) {
      return new Command({
        update: buildTerminationUpdate(
          graphState,
          outcome,
        ) as ManagerGraphUpdate,
        goto: END,
      });
    }
  }
  ...
  try {
    ...
    const dispatchError =
      caughtError instanceof AgentFailureError
        ? caughtError
        : new ApiTimeoutError("manager", "manager", 1, 500);
    const outcome = await failureHandler.dispatch(
      dispatchError,
      graphState,
      config,
    ...
    const policyUpdate: Partial<GraphState> = {
      degradationSignal: graphState.degradationSignal,
      reflexionContext: graphState.reflexionContext,
      activeToolOverride: graphState.activeToolOverride,
      qualityFlag: graphState.qualityFlag,
    };

    return new Command({
      update: policyUpdate as unknown as ManagerGraphUpdate,
      goto: END,
    });
  }
}
